...
void TreeDrawer::printTextTreeNodeHelper(ast* node)
{
	//cout << endl << hex << "0x" << (((int)node) >> 16 & 0xffff) << oct << endl;
	...
	if(node->nodetype & 0xff <= 7)
		...
		cout << (char)node->nodetype << "(";
	
	//cout << endl << hex << "left:0x" << node->l << ";right:0x" << node->r << oct << endl;

	if (node->l)
	{
		printTextTreeNodeHelper(node->l);
	}
	else
	{
		cout << "NULL";
	...
	if (node->r)
	{
		printTextTreeNodeHelper(node->r);
	}
	else
	{
		cout << "NULL";
...
void TreeDrawer::printTextTreeNode(ast* node)
{
	printTextTreeNodeHelper(node);
	cout << endl;
}
...
bool TreeDrawer::printTextTree()
{
	if (this->root == NULL)
		return false;

	this->printTextTreeNode(this->root);
	return true;
}

bool TreeDrawer::printTextTree(ast* node)
{
	if (node == NULL)
		return false;

	this->printTextTreeNode(node);
	return true;
}
...
void Levelise(ast* node, int k)
{
	...
	if (node->nodetype == '=')	//	赋值语句 symasgn
	{
		...
		singlestring* ii = new singlestring();
		ii->nodetype = 128;
		ii->l = NULL;
		ii->r = NULL;
		// this is dangerous!!!
