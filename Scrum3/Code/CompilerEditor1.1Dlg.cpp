...
BEGIN_MESSAGE_MAP(CCompilerEditor11Dlg, CDialogEx)
	...
	ON_COMMAND(ID_GENERATEIR, &CCompilerEditor11Dlg::OnGenerateir)
	ON_BN_CLICKED(IDC_BUTTON_OUTPUT, &CCompilerEditor11Dlg::OnBnClickedButtonOutput)
END_MESSAGE_MAP()
...
BOOL CCompilerEditor11Dlg::OnInitDialog()
{
	...
	CEdit* f = (CEdit*)(this->GetDlgItem(IDC_OUTFILENAME));
	f->SetWindowText("output.ll");
...
void CCompilerEditor11Dlg::OnOpenfile()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		while (true)
		{
			currPos = strPath.Find("\\", lastPos + 1);
			if (currPos == -1)
				break;
			lastPos = currPos;
		}
		...
		while (true)
		{
			currPoint = strPath.Find(".", lastPoint + 1);
			if (currPoint == -1)
				break;
			lastPoint = currPoint;
		}
		...
		// 生成.ll文件名
		CString tailFileName = "";
		for (; lastPos < lastPoint; lastPos++)
			tailFileName += strPath.GetAt(lastPos);
		saveFilePath = tailFileName + ".ll";
		
		CEdit* ofileArea = (CEdit*)(this->GetDlgItem(IDC_OUTFILENAME));
		ofileArea->SetWindowTextA(saveFilePath);
...
void CCompilerEditor11Dlg::OnSavefile()
{
	...
	CString strPath = openedFilePath;
	/* 生成output文件名 */
	// 寻找最后一个"反/"
	int lastPos = -1;
	int currPos = 0;
...
void CCompilerEditor11Dlg::OnOnsaveoutput()
{
	...
	CString str;
	CEdit* outputArea = (CEdit*)(this->GetDlgItem(IDC_EDIT_OUTPUTAREA));
	outputArea->GetWindowText(str);

	// 获取输出文件名字
	CEdit* outputFileNameArea = (CEdit*)(this->GetDlgItem(IDC_OUTFILENAME));
	CString fname = "";
	outputFileNameArea->GetWindowText(fname);
	...
	if (output == NULL)
	{
		outputArea->SetWindowText("保存文件失败！");
		return;
	}
	...
	CFile file("./" + fname, CFile::modeWrite | CFile::modeCreate);
	file.Write(output, length);
	file.Close();
	
	outputArea->SetWindowText("保存输出至"+ fname +"中");

}


void CCompilerEditor11Dlg::OnGenerateir()
{
	if(ifLex==true)ifLex = false;
	if(ifParse==true)ifParse = false;
	// TODO:  在此添加命令处理程序代码
	OnLexAnalyser();
	//OnParser();
	OnSementicanalyse();
...
void CCompilerEditor11Dlg::OnBnClickedButtonOutput()
{
	// TODO:  在此添加控件通知处理程序代码
	OnOnsaveoutput();
}
